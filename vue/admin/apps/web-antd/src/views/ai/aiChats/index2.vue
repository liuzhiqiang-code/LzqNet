...
const currentTitle = ref('Vue3 + Vben 开发规范调研')

const messages = ref<Message[]>([
  { id: 1, role: 'ai', content: '您好！我已经参考 Vben Admin 的设计语言重新调整了圆角逻辑：\n\n1. **组件化圆角**：采用了 Vben 标志性的 `16px-24px` 柔和圆角。\n2. **气泡一致性**：用户气泡（如 123）现在已完全圆角化。\n3. **模型器优化**：下拉框已美化并移至右侧。' },
])

const history = ref([
  { id: 1, title: '深度学习入门指南' },
  { id: 2, title: 'Vue3 + Vben 开发规范调研' }
])
        ...
        if (line.startsWith('title:')) {
          ...
          try {
            ...
            if (data.content) {
              ...
              // 同步更新侧边栏历史记录中的标题
              const histItem = history.value.find(h => h.id === 2) // 假设匹配当前ID
                ...
                <div class="relative model-selector">
                  ...
                  <transition name="vben-pop">
                    <div v-if="isModelOpen" class="absolute bottom-full mb-2 right-0 w-44 bg-[#1a1a1a] border border-[#333] rounded-xl shadow-2xl p-1.5 z-[50]">
                      <div v-for="model in ['Gemini 2.0 Flash', 'GPT-4o']" :key="model"
